...
public:
   ...
   static std::string show_times(const std::pair<std::uint64_t, std::vector<std::uint64_t>>& laps){
	   const std::vector<std::uint64_t> times = laps.second;
	   std::cout <<" time unit is based on template arg, default is millisec"<<std::endl;
	   std::cout << "Sum of (time) in all the recorded laps: " << laps.first <<std::endl;
	   std::string result("{");
